...
the runner and every benchmark already work with it. See `adapters/aide_adapter.py`
...
- **No docker required.** mle-bench ships a docker-based reference agent harness;
  we don't use it — we run the mle-bench *library* (prepare/grade) and AIDE
  natively, which is enough for the entry/exit contracts.

## LLM routing

AIDE routes to an OpenAI-compatible endpoint when `OPENAI_BASE_URL` is set and the
model name has no `gpt-`/`claude-`/`gemini-` prefix. `Kimi-K2.6` qualifies, so
`configure_aide_env("litellm")` (sets `OPENAI_BASE_URL` + `OPENAI_API_KEY`) is all
AIDE needs. Backends: `litellm` (Kimi, default), `claude_p`, `local` (Ollama).
