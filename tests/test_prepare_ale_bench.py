...
# and consumed on an older-glibc cluster (see _build_tools). Only two objdump
# outcomes are accepted; everything else must raise. --------------------------
...
# objdump prints this header for any file it successfully parses.
_HEADER = "t:     file format elf64-x86-64\n\n"
_NOT_DYNAMIC = "objdump: t: not a dynamic object\n"
...
# ---- shape 1: rc=0, parsed, no GLIBC symbols -> PASS ------------------------
...
def test_static_pie_binary_is_accepted(tmp_path, monkeypatch):
    ...
    This is what our musl toolchain actually emits."""
    _stub_subprocess(monkeypatch, stdout=_HEADER)
    prepare._assert_static(tmp_path / "tester")     # must not raise


# ---- shape 2: rc=1 + "not a dynamic object" -> PASS -------------------------

def test_static_non_pie_binary_is_accepted(tmp_path, monkeypatch):
    """A fully static NON-PIE binary has no dynamic table at all, so objdump
    exits 1 after parsing the file happily. Genuinely static, so it satisfies
    the guarantee and must PASS — keying on rc alone would refuse it."""
    _stub_subprocess(monkeypatch, stdout=_HEADER, stderr=_NOT_DYNAMIC,
                     returncode=1)
    prepare._assert_static(tmp_path / "tester")     # must not raise
...
def test_nonzero_exit_with_other_diagnostic_is_refused(tmp_path, monkeypatch):
    """rc=1 for a DIFFERENT reason is not the static case. objdump can print
    a header and still fail (truncated file, permission denied), and that
    must not be read as proof of a static binary."""
    _stub_subprocess(monkeypatch, stdout=_HEADER,
                     stderr="objdump: t: Permission denied\n", returncode=1)
    with pytest.raises(RuntimeError, match="could not verify static linkage"):
        prepare._assert_static(tmp_path / "tester")


def test_unexpected_exit_code_is_refused_even_when_parsed(tmp_path, monkeypatch):
    """The accepted set is closed: only rc 0 and the rc=1 static case."""
    _stub_subprocess(monkeypatch, stdout=_HEADER, stderr=_NOT_DYNAMIC,
                     returncode=139)
    with pytest.raises(RuntimeError, match="could not verify static linkage"):
        prepare._assert_static(tmp_path / "tester")
...
def test_failed_objdump_is_refused_not_read_as_clean(tmp_path, monkeypatch):
    ...
    _stub_subprocess(monkeypatch, stdout="", returncode=1,
                     stderr="objdump: t: file format not recognized\n")
    with pytest.raises(RuntimeError, match="could not verify static linkage"):
        prepare._assert_static(tmp_path / "tester")
...
# ---- the original purpose: refuse glibc-linked tools ------------------------
